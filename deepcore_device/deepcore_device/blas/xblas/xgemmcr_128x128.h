#define xgemmcr_128x128(grad,suffix)        \
__global__ void dk_xgemmcr_128x128##suffix( \
          char*              d_c  ,\
    ...
    *((float4*)&sst_base[0x0000])=__half42float4(p);\
    *((float4*)&sst_base[0x1000])=__half42float4(q);\
    __syncthreads();\
    SZERO64(c)\
    ...
    for( int k=qnc-8; k>0; k-=8 ){\
        ...
        *((float4*)&sst[0x0000])=__half42float4(p);\
        BOP8x8(c,&a[0],&b[0])\
        *((float4*)&sst[0x1000])=__half42float4(q);\
        asld=asld_base+ofs;          \
        bsld=bsld_base+ofs;          \
        __syncthreads();             \
        b[0]=*((float4*)&bsld[0x00]);\
...
xgemmcr_128x128(0,)
xgemmcr_128x128(1,_drelu)
xgemmcr_128x128(1,_xdrv)

#define xgemmcrx_128x128(grad,suffix)       \
__global__ void dk_xgemmcrx_128x128##suffix(\
          char*              d_c  ,\
    const char* __restrict__ d_a  ,\
    const char* __restrict__ d_b  ,\
    const char* __restrict__ d_x  ,\
    ...
    unsigned int elda=lda<<3;\
    unsigned int eldb=ldb<<3;\
    d_a+=(bz*qnc+slot)*lda+(sai<<3);\
    d_b+=(bz*qnc+slot)*ldb+(sbi<<3);\
    ...
    *((float4*)&sst_base[0x1000])=__half42float4(q);\
    ...
    unsigned int ofs=0x2000;      \
    for( int k=qnc-8; k>0; k-=8 ){\
        q=*((const __half4* __restrict__)(d_b+=eldb));\
...
xgemmcrx_128x128(0,)
xgemmcrx_128x128(1,_drelu)
xgemmcrx_128x128(1,_xdrv)