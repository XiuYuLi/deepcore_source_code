__global__ void dk_xfft8x8_r2c_split_perm( 
      float2*              d_c, 
      ...
      int                  is_grad )
{
    ...
    #pragma unroll
        for( int i=0; i<4; ++i ){
            if((i*2+0)<ny){ c[i].x=__half2float(d_r[0]); } d_r+=nx;
            if((i*2+1)<ny){ c[i].y=__half2float(d_r[0]); } d_r+=nx;
        } 
    } __syncthreads();
...
__global__ void dk_xfft8x8_r2c_split_perm_pad( 
      float2*              d_c, 
const __half* __restrict__ d_r, 
const float * __restrict__ d_RF, 
      int                  nx, 
