__global__ void dk_xfft8x8_r2c( float2* d_c, 
...
__global__ void dk_xfft8x8_r2c_ext( float2* d_c, 
    const __half* __restrict__ d_r, const float* __restrict__ d_RF, 
    unsigned int nx, unsigned int ny, unsigned int ldr, int n, unsigned int n_cells )
{
    const int brev[]={0,2,1,3};
    ...
    #pragma unroll
        for( int i=0; i<4; ++i ){
            if((2*i+0)<ny){ c[i].x=__half2float(d_r[0]); d_r+=nx; }
            if((2*i+1)<ny){ c[i].y=__half2float(d_r[0]); d_r+=nx; }
        }
    } __syncthreads();
    s_vfft8( c, spx, spy, s_RF, brev );
    s_hfft8( c, &smem[y*40+v*10+u*4], spx, s_RF, brev, x, u );
...
__global__ void dk_xfft8x8_r2c_pad( float2* d_c, 
    const __half* __restrict__ d_r, const float* __restrict__ d_RF, 
...
__global__ void dk_xfft8x8_r2c_flip( float2* d_c, 
    const __half* __restrict__ d_r, const float* __restrict__ d_RF, 
    unsigned int nx, unsigned int ny, unsigned int ldr, int n, unsigned int n_cells )
{
    const int brev[]={0,2,1,3};
